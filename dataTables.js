...
const db = new sqlite3.Database('connectingPG.db', sqlite3.OPEN_READWRITE, (dict, err) => {
  ...
  for(var key in dict)
  {
    db.run('INSERT INTO Houses(name, imgurl, description, address, founding) VALUES(?, ?, ?, ?, ?)', [dict[key].name, dict[key].imgurl, dict[key].description, dict[key].address, dict[key].founding]);
  }
})

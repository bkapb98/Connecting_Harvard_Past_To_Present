...
const db = new sqlite3.Database('connectingPG.db', sqlite3.OPEN_READWRITE, (dict, err) => {
  ...
  for(let key in dict)
  {
    db.run('INSERT INTO Events(houseId, date, eventName, description, source) VALUES(?, ?, ?, ?, ?)', [db.run('SELECT Houses.houseId FROM Houses WHERE Houses.Name = :name', name = dict[key].name), dict[key].date, dict[key].description, dict[key].name, dict[key].description, dict[key].source]);
  }
})})
